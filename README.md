...
# 制作背景
  ...
  gif画像 
  トップページ     https://gyazo.com/48eb76dc842ab2cfec2be5ca2ae99deb
 ...
 * コメント投稿機能を非同期通信で行っているのですが、最初はrailsの非同期では無い状態で実装していたのですが、
 全くjsが分からない状態から、jqueryを導入し、コメント投稿機能を調べ、何とか非同期でコメントを投稿する事ができるようになりました。
...
# 課題や今後実装したい機能
 ...
 ・ビューファイルは
  現状トップページは必要最小限の見た目を整えているに過ぎず、字体や動画にも工夫や改善点が多いなと考えております。思い出作成ページも殺風景で入力欄のみある状態でかつ左に偏っていたり綺麗に形になっているとは言えない状況なので改善していきたい。
...
# 使用技術
 ・非同期通信
 ・bootstrapによるビューファイルのデザインテンプレートの使用
 ・ActiveStorageによる複数枚の画像投稿機能や動画投稿機能
# テーブル設計
